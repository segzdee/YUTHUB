...
const fileUploadSchema = z.object({
  ...
  isConfidential: z.string().optional().transform(val => val === 'true'),
});
...
router.post('/upload', isAuthenticated, upload.array('files', 10), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/documents', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// Get single document
router.get('/documents/:id', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const hasAccess = await checkFileAccess(req.user!.id, documentId, req.user!.organizationId);
    if (!hasAccess) {
...
// Download file
router.get('/download/:id', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    const document = await storage.getDocument(documentId);
...
// Get thumbnail
router.get('/thumbnail/:id', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    const document = await storage.getDocument(documentId);
...
router.patch('/documents/:id', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    ...
    if (tags !== undefined) updates.tags = Array.isArray(tags) ? tags : tags.split(',').map((tag: string) => tag.trim());
...
// Delete document
router.delete('/documents/:id', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    const document = await storage.getDocument(documentId);
...
// Share document
router.post('/share', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { documentId, sharedWith, accessLevel, expiresAt, shareReason } = req.body;
...
// Get file shares
router.get('/shares/:documentId', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// Revoke file share
router.delete('/shares/:shareId', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/access-logs/:documentId', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const documentId = parseInt(req.params.documentId);
...
router.get('/analytics', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  try {
