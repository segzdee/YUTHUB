...
export default function UKCouncilAnalytics({ councils }: UKCouncilAnalyticsProps) {
  ...
  const regionData = useMemo(() => {
    const regions = councils.reduce((acc, council) => {
  ...
  const partnershipData = useMemo(() => {
    const partnerships = councils.reduce((acc, council) => {
      const metadata = councilMetadata[council.clientName];
  ...
  const monthlyOnboardingData = useMemo(() => {
    ...
    const data = months.map(month => ({ month, newClients: 0, totalRevenue: 0 }));
    ...
    <div className="space-y-6">
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={regionData}>
                  ...
                  <Bar dataKey="housingUnits" fill="#10B981" name="Housing Units" />
                </BarChart>
              </ResponsiveContainer>
            </div>
              ...
              {regionData.map((region, index) => (
                <div key={region.region} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{region.councils} councils</p>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold">{formatCurrency(region.revenue)}</p>
                    <p className="text-sm text-gray-600">{formatNumber(region.population)} people</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {partnershipData.map((partnership, index) => (
                <div key={partnership.status} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{partnership.count} councils</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold">{formatCurrency(partnership.revenue)}</p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {councils
              .sort((a, b) => parseFloat(b.defaultRate) - parseFloat(a.defaultRate))
              .slice(0, 5)
              .map((council, index) => {
                const metadata = councilMetadata[council.clientName];
                return (
                  <div key={council.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{metadata?.region}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold">{formatCurrency(parseFloat(council.defaultRate))}</p>
