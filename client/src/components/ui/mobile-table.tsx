...
import { ArrowDown, ArrowUp, ArrowUpDown, Home, Mail, Phone, Search, Users } from "lucide-react";
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        {isLoading ? (
          ...
          <>
            {/* Desktop table view */}
            <div className={`overflow-x-auto ${mobileCardView ? 'hidden sm:block' : ''}`}>
              <Table>
                <TableHeader>
                  <TableRow>
                    {columns.map((column) => (
...
export function YouthHousingTable() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Status',
      render: (value: string) => {
        const variant = value === 'Active Client' ? 'default' : 
                     value === 'Trial Period' ? 'secondary' : 'outline';
