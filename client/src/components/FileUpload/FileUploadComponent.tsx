...
import { Upload, File, Image, Download, Share2, Eye, Trash2, AlertCircle, Check, X } from 'lucide-react';
...
const FileUploadComponent: React.FC<FileUploadProps> = ({
  ...
  const { data: documents, isLoading: documentsLoading } = useQuery({
    queryKey: ['/api/files/documents', entityType, entityId, filterType, searchQuery],
    queryFn: async () => {
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span>{formatFileSize(storageAnalytics.totalSize)} / {formatFileSize(storageAnalytics.limit)}</span>
              </div>
              <Progress value={storageAnalytics.usagePercentage} className="h-2" />
              {storageAnalytics.usagePercentage > 80 && (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Storage usage is high. Consider upgrading your plan or cleaning up old files.
                  </AlertDescription>
                </Alert>
              )}
            </div>
        ...
        <TabsContent value="upload" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload documents, photos, and other files for {entityType} #{entityId}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button
                ...
                className="w-full"
              >
                {uploadMutation.isPending ? 'Uploading...' : `Upload ${uploadingFiles.length} file(s)`}
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            ...
                            {document.isConfidential && <Badge variant="secondary" className="ml-2">Confidential</Badge>}
                          </div>
                        ...
                        <Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Share "{selectedDocument?.originalName}" with another user
                              </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4">
                              ...
                              <div>
                                <Label htmlFor="shareReason">Reason for Sharing</Label>
                                <Textarea
                                  id="shareReason"
                                  value={shareReason}
                              ...
                              <Button
                                onClick={handleShare}
                                disabled={!shareUserId || !shareReason || shareMutation.isPending}
                                className="w-full"
                              >
