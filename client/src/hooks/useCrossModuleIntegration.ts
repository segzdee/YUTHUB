...
export function useCrossModuleIntegration() {
  ...
  const calculateMetrics = useMemo(() => {
    return () => {
      ...
      const totalCapacity = properties.reduce((sum, prop) => sum + (prop.capacity || 0), 0);
      const occupancyRate = totalCapacity > 0 ? (totalOccupancy / totalCapacity) * 100 : 0;

      const activeSupportPlans = supportPlans.filter(plan => plan.status === 'active').length;
      const highRiskResidents = residents.filter(resident => resident.riskLevel === 'high').length;
      const activeIncidents = incidents.filter(incident => incident.status === 'open').length;
      const pendingMaintenance = incidents.filter(incident => 
  ...
  }, [residents, properties, supportPlans, incidents, financialRecords, invoices]);
