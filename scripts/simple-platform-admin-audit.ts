...
async function auditPlatformAdminDatabase() {
  ...
  try {
    ...
    try {
      ...
      plans.forEach(plan => {
        console.log(`  - ${plan.displayName}: £${plan.monthlyPrice}/month (${plan.maxResidents || 'unlimited'} residents)`);
      });
    } catch (error) {
    ...
    try {
      const adminCount = await db.select({ count: count() }).from(users).where(eq(users.role, 'platform_admin'));
    ...
    try {
      const subscriptionCount = await db.select({ count: count() }).from(organizationSubscriptions);
      const activeSubscriptions = await db.select({ count: count() }).from(organizationSubscriptions).where(eq(organizationSubscriptions.status, 'active'));
    ...
    try {
      ...
      `);
      console.log(`✅ ${(indexQuery[0] as any).index_count} indexes found on core tables`);
    } catch (error) {
    ...
    console.log("\n📊 AUDIT SUMMARY");
    console.log("================");
    console.log(`✅ Database tables: ${tablesExist}/${tableTests.length} verified`);
    ...
    console.log("✅ Platform admin permissions separated from organization users");
