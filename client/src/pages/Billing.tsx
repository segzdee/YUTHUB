...
export default function Billing() {
  ...
  const filteredInvoices = invoices.filter(invoice => {
    const matchesSearch = invoice.invoiceNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
      governmentClients.find(c => c.id === invoice.clientId)?.name.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="flex-1 lg:ml-64 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
          <div className="mb-6 sm:mb-8">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Government Billing</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6 sm:mb-8">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{governmentClients.length}</div>
                <p className="text-xs text-muted-foreground">
                  Government clients
                </p>
              </CardContent>
          ...
          <Tabs defaultValue="invoices" className="space-y-6">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Badge className="bg-red-100 text-red-800">{overdueInvoices}</Badge>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium">{formatCurrency(totalRevenue + outstandingAmount)}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    <Card key={invoice.id} className="hover:shadow-md transition-shadow">
                      ...
                      <CardContent>
                        <div className="flex justify-between items-center">
                          <div className="text-sm text-gray-600 dark:text-gray-400">
                            {invoice.description || "Government billing invoice"}
                          </div>
                          <div className="flex gap-2">
            ...
            <TabsContent value="clients" className="space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Government Clients</h2>
                <Button className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  Add Client
                </Button>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <CardDescription>{client.department}</CardDescription>
                          </div>
                        </div>
            ...
            <TabsContent value="rates" className="space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Support Level Rates</h2>
                <Button className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  Add Rate
                </Button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {supportLevelRates.map((rate) => (
                  <Card key={rate.id} className="hover:shadow-md transition-shadow">
                    <CardHeader>
                      <CardTitle className="text-lg">{rate.supportLevel}</CardTitle>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <div className="font-medium">{formatCurrency(rate.weeklyRate)}</div>
                          </div>
                          <div>
                            <span className="text-gray-600 dark:text-gray-400">Monthly</span>
                            <div className="font-medium">{formatCurrency(rate.monthlyRate)}</div>
                          </div>
                        </div>
                      </div>
                    </CardContent>
            ...
            <TabsContent value="periods" className="space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Billing Periods</h2>
                <Button className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  Create Period
                </Button>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <CardDescription>
                              {getClientName(period.clientId)} • {residents.find(r => r.id === period.residentId)?.firstName} {residents.find(r => r.id === period.residentId)?.lastName}
                            </CardDescription>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-lg font-bold">{formatCurrency(period.totalAmount)}</div>
                          <div className="text-sm text-gray-600 dark:text-gray-400">
            ...
            <TabsContent value="analytics" className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {billingAnalytics?.topClients?.map((client: any, index: number) => (
