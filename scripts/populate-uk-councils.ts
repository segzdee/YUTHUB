...
async function populateUKCouncils() {
  try {
    ...
    // Load the JSON data
    const dataPath = path.join(__dirname, '..', 'data', 'uk-borough-councils.json');
    ...
    console.log(`Total Population: ${jsonData.summary_statistics.total_population.toLocaleString()}`);
    console.log(`Total Social Housing Units: ${jsonData.summary_statistics.total_social_housing_units.toLocaleString()}`);
    console.log(`Average Units per Council: ${jsonData.summary_statistics.average_units_per_council.toLocaleString()}`);
    
    console.log('\n🤝 Partnership Status:');
    console.log(`Active Clients: ${jsonData.summary_statistics.partnership_breakdown.active_clients}`);
    console.log(`Trial Period: ${jsonData.summary_statistics.partnership_breakdown.trial_period}`);
    console.log(`Prospective Clients: ${jsonData.summary_statistics.partnership_breakdown.prospective_clients}`);
    
    console.log('\n🗺️  Geographic Distribution:');
    Object.entries(jsonData.summary_statistics.geographic_distribution).forEach(([region, count]) => {
