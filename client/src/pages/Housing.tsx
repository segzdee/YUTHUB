...
import { Building, Users, Wrench, Plus, MapPin, Calendar, AlertTriangle } from "lucide-react";
...
export default function Housing() {
  ...
  const getOccupancyRate = (property: Property) => {
    return property.totalUnits > 0 ? (property.occupiedUnits / property.totalUnits) * 100 : 0;
  };
  ...
  const getPropertyTypeLabel = (type: string) => {
    switch (type) {
      case "shared_housing": return "Shared Housing";
      case "studio_units": return "Studio Units";
      case "transition_units": return "Transition Units";
      default: return type;
    }
  };
      ...
      <div className="flex-1 lg:ml-64 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
          <div className="mb-6 sm:mb-8">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Housing Management</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Manage properties, track occupancy, and coordinate maintenance requests
            </p>
          </div>
          ...
          <Tabs defaultValue="properties" className="space-y-6">
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm font-medium">{getPropertyTypeLabel(property.propertyType)}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600 dark:text-gray-400">Occupancy</span>
                          <span className="text-sm font-medium">
                            {property.occupiedUnits}/{property.totalUnits} ({getOccupancyRate(property).toFixed(0)}%)
                          </span>
                        </div>
            ...
            <TabsContent value="residents" className="space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Current Residents</h2>
                <Button className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  Add Resident
                </Button>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <CardDescription>
                              {resident.propertyId && properties.find(p => p.id === resident.propertyId)?.name}
                            </CardDescription>
                          </div>
                        </div>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm font-medium">{resident.unitNumber || "N/A"}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600 dark:text-gray-400">Independence Level</span>
                          <span className="text-sm font-medium">{resident.independenceLevel}/5</span>
                        </div>
                        <div className="flex items-center justify-between">
