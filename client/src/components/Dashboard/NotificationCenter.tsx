...
export default function NotificationCenter({
  ...
  useEffect(() => {
    ...
    const highRiskResidents = incidents.filter(
      i => i.severity === 'high'
    ).length;
    const activeIncidents = incidents.filter(i => i.status === 'open').length;
    const overdueInvoices = financialRecords.filter(
      r => r.status === 'overdue'
    ).length;
    const pendingInvoices = financialRecords.filter(
      r => r.status === 'pending'
    ).length;

    // Housing Officer notifications
    if (
      userRole === 'housing_officer' ||
      userRole === 'admin' ||
      userRole === 'manager'
    ) {
      if (occupancyRate > 95) {
