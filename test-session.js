...
async function testSessionPersistence() {
  ...
  try {
    ...
    const result = await pool.query('SELECT COUNT(*) as count FROM sessions WHERE expire > NOW()');
    console.log('Active sessions in database:', result.rows[0].count);
    
    // Get a sample session
    const sessionResult = await pool.query('SELECT sid, expire, sess FROM sessions WHERE expire > NOW() ORDER BY expire DESC LIMIT 1');
