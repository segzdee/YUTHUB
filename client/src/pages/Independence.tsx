...
import { TrendingUp, Target, Calendar, CheckCircle, Plus, User, Award, Clock } from "lucide-react";
...
export default function Independence() {
  ...
  const skillAreas = [
    { name: "Life Skills", icon: Target, description: "Cooking, cleaning, budgeting, time management" },
    { name: "Employment", icon: Award, description: "Job searching, CV writing, interview skills" },
    { name: "Education", icon: TrendingUp, description: "Academic support, training courses, certifications" },
    { name: "Health & Wellbeing", icon: CheckCircle, description: "Mental health, physical health, self-care" },
    { name: "Social Skills", icon: User, description: "Communication, relationships, community engagement" },
    { name: "Housing", icon: Calendar, description: "Tenancy skills, property maintenance, housing applications" },
  ];

  // Calculate average independence level
  const avgIndependenceLevel = residents.length > 0 
    ? residents.reduce((sum, resident) => sum + resident.independenceLevel, 0) / residents.length 
    : 0;
      ...
      <div className="flex-1 lg:ml-64 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
          <div className="mb-6 sm:mb-8">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Independence Pathway</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Track resident progress, manage goals, and support skill development
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6 sm:mb-8">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Average Independence</CardTitle>
                <TrendingUp className="h-4 w-4 text-primary" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{avgIndependenceLevel.toFixed(1)}/5</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{progressTracking.filter(p => p.status === "on_track").length}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{progressTracking.filter(p => p.status === "achieved").length}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{progressTracking.filter(p => p.status === "at_risk").length}</div>
                <p className="text-xs text-muted-foreground">
                  Requiring attention
                </p>
              </CardContent>
          ...
          <Tabs defaultValue="residents" className="space-y-6">
            ...
            <TabsContent value="residents" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredResidents.map((resident) => {
                  const residentProgress = progressTracking.filter(p => p.residentId === resident.id);
                  const activeGoals = residentProgress.filter(p => p.status === "on_track");
                  const achievedGoals = residentProgress.filter(p => p.status === "achieved");
                        ...
                        <div className="space-y-4">
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium">Independence Progress</span>
                              <span className="text-sm text-gray-600 dark:text-gray-400">
                                {resident.independenceLevel * 20}%
                              </span>
                            </div>
                            <Progress value={resident.independenceLevel * 20} className="h-2" />
                          </div>
            ...
            <TabsContent value="goals" className="space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Goals & Milestones</h2>
                <Button className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  New Goal
                </Button>
                      ...
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <CardTitle className="text-lg">{progress.goalTitle}</CardTitle>
                              <CardDescription>
                                {getResidentName(progress.residentId)} • {progress.skillArea}
                              </CardDescription>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="text-sm font-medium">
                                {progress.targetDate ? new Date(progress.targetDate).toLocaleDateString() : "No target"}
                              </div>
                            </div>
                            <div>
            ...
            <TabsContent value="skills" className="space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Skill Development Areas</h2>
                <Button className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  Add Skill Area
                </Button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {skillAreas.map((skill) => {
                  ...
                  const skillProgress = progressTracking.filter(p => p.skillArea === skill.name);
                  const avgProgress = skillProgress.length > 0 
                    ? skillProgress.reduce((sum, p) => sum + p.progressPercentage, 0) / skillProgress.length 
                    : 0;
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="text-center">
                              <div className="text-2xl font-bold text-green-600">
                                {skillProgress.filter(p => p.status === "achieved").length}
                              </div>
                              <div className="text-xs text-gray-600 dark:text-gray-400">Achieved</div>
                            </div>
                          </div>
