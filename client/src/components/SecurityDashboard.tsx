...
export default function SecurityDashboard() {
  ...
  const calculateSecurityScore = (metrics: SecurityMetrics) => {
    ...
    const mfaAdoption =
      metrics.mfaEnabled / Math.max(metrics.activeSessions, 1);
          ...
          <div className='flex items-center space-x-4'>
            <div className='flex-1'>
              <div className='flex items-center justify-between mb-2'>
                ...
                <Badge
                  className={`${securityLevel.bgColor} ${securityLevel.color}`}
                >
                  {securityLevel.level}
                </Badge>
              </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-muted-foreground'>
              {(
                ((metrics?.failedLogins || 0) /
                  Math.max(metrics?.totalLogins || 1, 1)) *
                100
              ).toFixed(1)}
              % failure rate
            </p>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold text-green-600'>
              {metrics?.mfaEnabled || 0}
            </div>
            <p className='text-xs text-muted-foreground'>
              {(
                ((metrics?.mfaEnabled || 0) /
                  Math.max(metrics?.activeSessions || 1, 1)) *
                100
              ).toFixed(1)}
              % adoption rate
            </p>
