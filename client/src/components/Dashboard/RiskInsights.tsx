        ...
        <div className='space-y-4'>
              ...
              <div className='flex items-start justify-between'>
                <div className='flex-1'>
                  ...
                  <p className='text-sm text-neutral-600'>
                    Property ID: {resident.propertyId} • Key Worker:{' '}
                    {resident.keyWorkerId}
                  </p>
                  <p
                    className={`text-xs mt-2 ${getRiskBadgeColor(resident.riskLevel)}`}
                  >
                    Risk Level:{' '}
                    {resident.riskLevel.charAt(0).toUpperCase() +
                      resident.riskLevel.slice(1)}
                  </p>
                </div>
                <Button
                  variant='ghost'
          ...
          {(!riskyResidents || riskyResidents.length === 0) && (
            <div className='text-center py-8 text-neutral-500'>
              ...
              <p className='text-sm'>
                AI monitoring is active and all residents are in good standing
              </p>
            </div>
          )}
        </div>
