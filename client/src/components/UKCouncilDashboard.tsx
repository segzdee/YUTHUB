...
export default function UKCouncilDashboard() {
  ...
  const [selectedCouncil, setSelectedCouncil] = useState<GovernmentClient | null>(null);
  ...
  const filteredCouncils = useMemo(() => {
    return councils.filter(council => {
      const matchesSearch = council.clientName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      const matchesRegion = regionFilter === 'all' || (metadata && metadata.region === regionFilter);
  ...
  const regions = useMemo(() => {
    const regionSet = new Set(Object.values(councilMetadata).map(c => c.region));
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        <p className="text-red-600">Error loading council data. Please try again.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatNumber(totalHousingUnits)}</div>
            <p className="text-xs text-gray-600">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(
                filteredCouncils.reduce((sum, council) => sum + parseFloat(council.defaultRate), 0).toString()
              )}
            </div>
            <p className="text-xs text-gray-600">
                ...
                return (
                  <TableRow key={council.id}>
                    <TableCell className="font-medium">
                      <div>
                        <div className="font-medium">{council.clientName}</div>
                        <div className="text-sm text-gray-600">{metadata?.area}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>{formatNumber(metadata?.population || 0)}</TableCell>
                    <TableCell>{formatNumber(metadata?.socialHousingUnits || 0)}</TableCell>
                    <TableCell>{formatCurrency(council.defaultRate)}</TableCell>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-start mb-6">
                <h2 className="text-2xl font-bold">{selectedCouncil.clientName}</h2>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Card>
                      ...
                      <CardContent className="space-y-2">
                        <p><strong>Type:</strong> {selectedCouncil.clientType}</p>
                        <p><strong>Region:</strong> {councilMetadata[selectedCouncil.clientName]?.region}</p>
                        <p><strong>Population:</strong> {formatNumber(councilMetadata[selectedCouncil.clientName]?.population || 0)}</p>
                        <p><strong>Housing Units:</strong> {formatNumber(councilMetadata[selectedCouncil.clientName]?.socialHousingUnits || 0)}</p>
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-2">
                        <p><strong>Status:</strong> {getStatusBadge(selectedCouncil)}</p>
                        <p><strong>Join Date:</strong> {councilMetadata[selectedCouncil.clientName]?.joinDate || 'Not specified'}</p>
                        <p><strong>Housing Officer:</strong> {councilMetadata[selectedCouncil.clientName]?.housingOfficer}</p>
                        <p><strong>Active:</strong> {selectedCouncil.isActive ? 'Yes' : 'No'}</p>
                      </CardContent>
                    </Card>
                  </div>
                </TabsContent>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex items-start space-x-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{selectedCouncil.billingAddress}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                <TabsContent value="billing" className="space-y-4">
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <p><strong>Default Rate:</strong> {formatCurrency(selectedCouncil.defaultRate)}</p>
                      <p><strong>Payment Terms:</strong> {selectedCouncil.paymentTerms} days</p>
                      <p><strong>Client Type:</strong> {selectedCouncil.clientType}</p>
                      <p><strong>Created:</strong> {new Date(selectedCouncil.createdAt).toLocaleDateString()}</p>
                      <p><strong>Last Updated:</strong> {new Date(selectedCouncil.updatedAt).toLocaleDateString()}</p>
                    </CardContent>
                  </Card>
                </TabsContent>
              </Tabs>
