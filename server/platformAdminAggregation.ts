...
export class PlatformDataAggregator {
  ...
  static async getPlatformOverview(): Promise<{
    ...
    try {
      ...
      const [
        ...
        db.select({ count: count() }).from(residents).where(eq(residents.status, 'active')),
        
        // Total properties
        db.select({ count: count() }).from(properties).where(eq(properties.status, 'active')),
        
        // Total incidents (last 30 days)
        db.select({ count: count() }).from(incidents).where(
          gte(incidents.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
        ),
        
        // Total revenue from active subscriptions
        db.select({ 
          total: sql<number>`COALESCE(SUM(CAST(${organizationSubscriptions.amount} AS DECIMAL)), 0)` 
        }).from(organizationSubscriptions).where(eq(organizationSubscriptions.status, 'active')),
        ...
        db.select({ count: count() }).from(organizationSubscriptions).where(
      ...
      const occupancyRate = occupancyData[0].totalCapacity > 0 
        ? (occupancyData[0].occupiedUnits / occupancyData[0].totalCapacity) * 100 
        : 0;
  ...
  static async getOrganizationBreakdowns(timeRange: string = '30d'): Promise<Array<{
      ...
      const organizationMetrics = await Promise.all(
        orgs.map(async (org) => {
          const [
            ...
            // Residents count
            db.select({ count: count() }).from(residents).where(
              and(
            ...
            db.select({ 
              total: sql<number>`COALESCE(SUM(CAST(${organizationSubscriptions.amount} AS DECIMAL)), 0)` 
            }).from(organizationSubscriptions).where(
              and(
          ...
          return {
            ...
            revenue: parseFloat(subscriptionRevenue[0].total?.toString() || '0'),
            occupancyRate: Math.round(occupancyRate * 100) / 100,
  ...
  static async getHistoricalTrends(months: number = 12): Promise<{
    ...
    try {
      ...
      for (let i = 0; i < months; i++) {
        ...
        const occupancyRate = occupancy[0].totalCapacity > 0 
      ...
      return {
        residentTrends: monthlyData.map(d => ({ month: d.month, count: d.residents })).reverse(),
        revenueTrends: monthlyData.map(d => ({ month: d.month, revenue: d.revenue })).reverse(),
        incidentTrends: monthlyData.map(d => ({ month: d.month, incidents: d.incidents })).reverse(),
        occupancyTrends: monthlyData.map(d => ({ month: d.month, occupancy: d.occupancy })).reverse()
      };
    } catch (error) {
  ...
  static async getRealTimeMetrics(): Promise<{
    ...
    try {
      const [
        ...
        }).from(maintenanceRequests).where(
          gte(maintenanceRequests.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
        ),
        
        // Critical incidents (last 7 days)
        db.select({ count: count() }).from(incidents).where(
          and(
            gte(incidents.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)),
            eq(incidents.severity, 'critical')
          )
        ),
        ...
        }).from(supportPlans).where(
          gte(supportPlans.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
        ),
      ...
      const supportPlanCompletion = supportPlanStats[0].total > 0 
        ? (supportPlanStats[0].completed / supportPlanStats[0].total) * 100 
        : 0;

      return {
        activeUsers: activeUsers[0].count,
        totalMaintenanceCosts: parseFloat(maintenanceCosts[0].total?.toString() || '0'),
...
export class AggregatedDataValidator {
  ...
  static async validateDataConsistency(): Promise<{
    ...
    try {
      ...
      const orphanedResidents = await db.select({ count: count() })
        .from(residents)
      ...
      if (orphanedResidents[0].count > 0) {
        issues.push(`${orphanedResidents[0].count} residents have invalid organization references`);
      }

      const orphanedProperties = await db.select({ count: count() })
        .from(properties)
        .leftJoin(organizations, eq(properties.organizationId, organizations.id))
        .where(sql`${organizations.id} IS NULL`);

      if (orphanedProperties[0].count > 0) {
        issues.push(`${orphanedProperties[0].count} properties have invalid organization references`);
      }

      // Check subscription consistency
      const invalidSubscriptions = await db.select({ count: count() })
      ...
      if (invalidSubscriptions[0].count > 0) {
        issues.push(`${invalidSubscriptions[0].count} subscriptions have invalid organization references`);
      }
  ...
  static async checkAggregationPerformance(): Promise<{
    ...
    try {
      ...
      const tests = [
        { name: 'Platform Overview', test: () => PlatformDataAggregator.getPlatformOverview() },
        { name: 'Organization Breakdowns', test: () => PlatformDataAggregator.getOrganizationBreakdowns() },
        { name: 'Real-time Metrics', test: () => PlatformDataAggregator.getRealTimeMetrics() }
      ];
...
export class SecureDataExporter {
  ...
  static async exportAggregatedReport(format: 'csv' | 'json' = 'json'): Promise<{
    data: any;
    filename: string;
  }> {
    try {
